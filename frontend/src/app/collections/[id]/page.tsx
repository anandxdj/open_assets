...
export default function CollectionDetailPage() {
  ...
  if (error && !collection) {
    return (
      <div className="min-h-screen bg-[#050506] grid place-items-center font-mono text-center gap-4 px-6">
        <Boxes className="h-10 w-10 text-zinc-700" />
        <p className="text-sm text-zinc-400 uppercase tracking-wider">{error}</p>
  ...
  return (
    <div className="min-h-screen bg-[#050506] text-zinc-100 font-mono">
      <Navbar />
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="mb-6">
          <Link
            href="/collections"
            className="inline-flex items-center gap-1.5 text-[10px] text-zinc-500 hover:text-white uppercase font-black tracking-widest transition-colors"
          >
        ...
        {/* Sidebar */}
        <aside className="lg:sticky lg:top-20 self-start border border-zinc-800 bg-zinc-950 rounded-md p-5 h-fit">
          <h1 className="text-xl font-black uppercase tracking-tight">{collection.name}</h1>
          <p className="text-[10px] text-zinc-500 mt-1 uppercase tracking-wider">by {creatorName(collection)}</p>
          {collection.description && (
            <p className="text-xs text-zinc-400 mt-3 leading-relaxed">{collection.description}</p>
          )}

          {collection.tags.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-4">
              {collection.tags.map((t) => (
                <span key={t} className="text-[9px] px-1.5 py-0.5 bg-zinc-900 border border-zinc-800 text-zinc-400 rounded uppercase tracking-wider">
                  {t}
                </span>
              ))}
            </div>
          ...
          <div className="flex flex-col gap-2 mt-5">
            ...
            <button
              onClick={handleLike}
              disabled={liking}
              className="py-2.5 px-4 bg-zinc-900 text-zinc-300 font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-1.5 hover:bg-zinc-800 hover:text-white border border-zinc-700 transition-all rounded disabled:opacity-50"
            >
        ...
        <section className="flex flex-col gap-4">
          {collection.folders.map((folder) => {
            ...
            return (
              <div key={folder._id} className="border border-zinc-800 bg-zinc-950 rounded-md overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-900">
                  <button
                    onClick={() => toggleFolder(folder._id)}
                    className="flex items-center gap-2 text-sm font-black uppercase tracking-wide hover:text-[#00ff66] transition-colors"
                  >
                    <ChevronDown className={cn("h-4 w-4 transition-transform", !isOpen && "-rotate-90")} />
                    {folder.name}
                    <span className="text-[10px] text-zinc-600">({folder.images.length})</span>
                  </button>
                  <a
                    href={folderDownloadUrl(id, folder._id)}
                    className="text-[9px] text-zinc-400 hover:text-[#00ff66] uppercase font-bold tracking-wider flex items-center gap-1 transition-colors"
                  >
                  ...
                  <div className="p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                    {folder.images.map((img) => (
                      <button
                        ...
                        className="group aspect-square border border-zinc-800 bg-zinc-900/40 rounded overflow-hidden hover:border-[#00ff66]/60 transition-colors relative"
                        style={{
                          backgroundImage:
      ...
      {preview && (
        ...
        >
          <div
            className="bg-[#0a0a0c] border border-zinc-800 rounded-lg max-w-3xl w-full max-h-[88vh] overflow-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between px-5 py-3 border-b border-zinc-900">
              <h3 className="text-sm font-black uppercase tracking-wide truncate">{preview.name}</h3>
              <button onClick={() => setPreview(null)} className="text-zinc-500 hover:text-white transition-colors">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="grid md:grid-cols-[1fr_240px] gap-0">
              ...
              <div className="p-5 border-t md:border-t-0 md:border-l border-zinc-900 flex flex-col gap-3">
                ...
                {preview.geminiMetadata?.description && (
                  <div>
                    <p className="text-[9px] text-zinc-600 uppercase tracking-widest">AI description</p>
                    <p className="text-xs text-zinc-300 mt-1 leading-relaxed">{preview.geminiMetadata.description}</p>
                  </div>
                )}
                ...
                <button
                  ...
                  className={cn(
                    "mt-auto py-2.5 px-4 font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-1.5 border transition-all rounded",
                    downloading
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="border border-zinc-800 bg-zinc-900/40 rounded py-2">
...
function Meta({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className="text-xs text-zinc-300 font-bold">{value}</span>
    </div>
  );
}
