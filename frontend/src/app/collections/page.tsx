  ...
  return (
    <div className="flex items-center gap-1.5 mt-1.5">
      {picture ? (
        ...
        <img src={picture} alt={name} className="h-4 w-4 rounded-full object-cover border border-zinc-700" />
      ) : (
        <span className="h-4 w-4 rounded-full grid place-items-center bg-zinc-800 text-[8px] font-black text-[#00ff66] border border-zinc-700">
          {name.charAt(0).toUpperCase()}
        </span>
      )}
      <span className="text-[10px] text-zinc-400 uppercase tracking-wider truncate">{name}</span>
    </div>
  );
}
...
function CoverCollage({ urls }: { urls: string[] }) {
  ...
  if (cells.length === 0) {
    return (
      <div className="aspect-[4/3] w-full grid place-items-center bg-zinc-900/60 border-b border-zinc-800">
        <Boxes className="h-8 w-8 text-zinc-700" />
      </div>
    );
  }
  return (
    <div className="aspect-[4/3] w-full grid grid-cols-2 grid-rows-2 gap-px bg-zinc-800 border-b border-zinc-800 overflow-hidden">
      {cells.map((u, i) => (
        // eslint-disable-next-line @next/next/no-img-element
        <img key={i} src={u} alt="" className="h-full w-full object-cover bg-zinc-950" loading="lazy" />
      ))}
    </div>
  );
}
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-8">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-zinc-950 border border-zinc-800 focus:border-[#00ff66] outline-none rounded px-9 py-2.5 text-sm placeholder:text-zinc-600 transition-colors"
            />
          </div>
          <div className="flex items-center gap-1.5">
            {SORTS.map((s) => (
              <button
                ...
                className={cn(
                  "text-[10px] px-3 py-2.5 border uppercase font-bold tracking-wider rounded transition-colors",
                  sort === s.key
        ...
        {loading ? (
          ...
          <div className="grid place-items-center py-24 text-center gap-3">
            ...
            <p className="text-sm text-zinc-500 uppercase tracking-wider">No collections found</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {items.map((c) => (
              <Link
                ...
                className="group border border-zinc-800 bg-zinc-950 rounded-md overflow-hidden hover:border-[#00ff66]/60 hover:-translate-y-1 transition-all duration-200"
              >
                <CoverCollage urls={c.coverImageUrls} />
                <div className="p-4">
                  <h3 className="text-sm font-black uppercase tracking-wide truncate group-hover:text-[#00ff66] transition-colors">
                    {c.name}
                  </h3>
                  ...
                  {c.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2.5">
                      {c.tags.slice(0, 3).map((t) => (
                        <span
                          key={t}
                          className="text-[9px] px-1.5 py-0.5 bg-zinc-900 border border-zinc-800 text-zinc-400 rounded uppercase tracking-wider"
                        >
                          {t}
                        </span>
                      ))}
