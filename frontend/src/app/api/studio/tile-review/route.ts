...
export async function POST(request: NextRequest) {
  try {
    const { prompt, sceneBrief, model, previewImage, sheetImage } =
      await request.json()
    ...
    const auth = await resolveKeyAndCredits(request, 'tile-review', modelId, 1)
    if (!auth.ok) {
...
Review the attached platform preview${
    ...
    if (!result.ok) {
      ...
      return NextResponse.json(
        { error: result.error || 'Failed to review tileset' },
