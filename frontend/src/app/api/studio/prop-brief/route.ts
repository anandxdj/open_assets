...
export async function POST(request: NextRequest) {
  try {
    const { prompt, sceneBrief, artStyle, model, count, existing } =
      await request.json()
    ...
    const auth = await resolveKeyAndCredits(request, 'prop-brief', modelId, 1)
    if (!auth.ok) {
      return NextResponse.json({ error: auth.error, code: auth.code }, { status: auth.status })
    }

    if (isMockMode()) {
      return NextResponse.json({ ideas: Array.from({ length: typeof count === 'number' ? count : 8 }, (_, i) => ({ category: `mock prop ${i + 1}`, description: 'a small mock decoration sprite on a flat magenta background' })) })
    }
