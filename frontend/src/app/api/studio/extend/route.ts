...
// Directional outpainting proxy. The client sends an expanded canvas with a
// light-gray blank area; the model fills it. Hybrid key + credits via _lib.
...
// GPT image models can take ~4 min per call.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const auth = await resolveKeyAndCredits(request, 'extend', modelId, 1)
    if (!auth.ok) {
      return NextResponse.json({ error: auth.error, code: auth.code }, { status: auth.status })
    }

    // Mock mode: deterministic fixture sized to the request, after the
    // key/credits gate so tests exercise the full auth path.
    if (isMockMode()) {
      const isKeyed = typeof layerRole === 'string' && layerRole !== 'sky'
      let w = 1024
      let h = 1024
      if (extensionInfo?.newWidth && extensionInfo?.newHeight) {
        w = extensionInfo.newWidth
        h = extensionInfo.newHeight
      } else if (chunkInfo) {
        w =
      ...
      const imageUrl = isKeyed ? mockKeyedPngDataUrl(w, h) : mockPngDataUrl(w, h)
      return NextResponse.json({ imageUrl, chunkInfo })
    }

    // ── Prompt construction (ported verbatim from upstream) ────────────────
...
6. Preserve the exact style, quality, and atmosphere of the existing content`
    ...
    const artStyleDescriptions: { [key: string]: string } = {
      ...
      'studio-ghibli':
        'Studio Ghibli animation style with whimsical, hand-drawn aesthetics and rich environmental details',
      pixar:
        'Pixar animation style with smooth 3D rendering, expressive characters, and vibrant colors',
      disney:
        'Disney animation style with classic hand-drawn or modern 3D aesthetics and magical atmosphere',
      dreamworks: 'DreamWorks animation style with dynamic expressions and cinematic lighting',
      illumination:
        'Illumination Entertainment style with bright colors, playful characters, and bold shapes',
      laika: 'Laika Studios stop-motion style with intricate textures and handcrafted details',
      'cartoon-network':
        'Cartoon Network style with bold outlines, simplified shapes, and vibrant colors',
      nickelodeon:
        'Nickelodeon animation style with energetic, expressive characters and bright color palettes',
      aardman:
        'Aardman claymation style with textured plasticine characters and British humor aesthetics',
      'blue-sky':
        'Blue Sky Studios animation style with detailed 3D rendering and dynamic action sequences',
    ...
    // Parallax layer extensions: keyed layers must keep the magenta key pure,
    // sky must stay opaque and horizontally uniform for tileability.
...
- Do NOT change the magenta background color in any region — it must stay pure ${KEY_COLOR_HEX} everywhere outside the elements, both in the existing area and in the new area.`
    ...
    if (!imageUrl) {
      console.error(
        '[studio/extend] no image in response:',
        JSON.stringify(sanitizeForLogging(message), null, 2),
      )
      if (!auth.byok && auth.eventId) await refundCredits(auth.eventId)
    ...
    console.error('[studio/extend] error:', error)
    return NextResponse.json(
