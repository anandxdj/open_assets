...
import { isMockMode, reconcileUsage, refundCredits, resolveKeyAndCredits } from "../../../studio/_lib/openrouter";
import { callLlm, providerHeaders, servedModel, LLM_LONG_BUDGET_MS } from "../../../studio/_lib/llm";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // One charge per interview round: `ask` rounds and the final `write` turn
    // are each a separate provider call, so each is a separate unit of work.
    const auth = await resolveKeyAndCredits(request, "anibuddy-prompt", ANIBUDDY_TEXT_MODEL, 1); if (!auth.ok) return NextResponse.json({ error: auth.error, code: auth.code }, { status: auth.status });
    ...
    // BYOK requests are OpenRouter-only — the user's key is an OpenRouter
    // credential and Open Quota is keyed from server env, so routing a BYOK
    // call there would silently spend the platform's quota for free.
    const result = await callLlm({ byok: auth.byok, openQuotaOnly: !auth.byok, openQuotaModel: ANIBUDDY_TEXT_MODEL, openQuotaFallbackModel: ANIBUDDY_TEXT_FALLBACK_MODEL, responseFormat: action === "ask" ? INTERVIEW_RESPONSE_FORMAT : undefined, key: auth.key, model: ANIBUDDY_TEXT_MODEL, messages: [{ role: "system", content: system }, { role: "user", content: `Character idea: ${body.idea.trim()}\n\nInterview so far:\n${history || "(none)"}` }], maxTokens: action === "ask" ? 700 : 400, temperature: action === "ask" ? 0.3 : 0.4, title: "AniBuddy - Concept interview", referer: request.headers.get("referer"), signal: request.signal, budgetMs: LLM_LONG_BUDGET_MS });
    ...
    const recordServedModel = async () => { if (!auth.byok && auth.eventId) await reconcileUsage(auth.eventId, servedModel(result, ANIBUDDY_TEXT_MODEL), result.provider); };
    if (action === "write") { const prompt = cleanPrompt(textOf(result.data.choices?.[0]?.message?.content)); if (prompt.length < 40) { if (!auth.byok && auth.eventId) await refundCredits(auth.eventId); return NextResponse.json({ error: "The model returned no usable prompt" }, { status: 502 }); } await recordServedModel(); return NextResponse.json({ prompt }, { headers: providerHeaders(result) }); }
