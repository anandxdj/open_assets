...
export default function LoginPage() {
  ...
  return (
    <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-16 items-center">
      ...
      {/* Right panel - Brutalist login card */}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <Link href="/forgot-password" className="text-[9px] text-muted-foreground hover:text-foreground hover:underline transition-all uppercase tracking-wide">
                Forgot key?
              </Link>
            </div>
            <input
          ...
          >
            {loading ? "AUTHENTICATING..." : "SUBMIT_CREDENTIALS →"}
          </button>
        </form>

        <p className="text-center text-[10px] text-muted-foreground uppercase tracking-wider">
          No account?{" "}
          <Link href="/register" className="font-black text-foreground hover:underline underline-offset-4 decoration-2">
            SIGN UP
          </Link>
        </p>
      </div>
